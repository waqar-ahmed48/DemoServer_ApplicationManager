...
#	kubectl exec vault-0 --namespace vault-ns -- vault operator seal || true
	helm delete demoserver-applicationmanager --namespace demoserver --wait || true
	...
	kubectl delete -n demoserver secret demoserver-applicationmanager || true
...
#	bring up stack
	...
	helm install -n demoserver my-postgres-release oci://registry-1.docker.io/bitnamicharts/postgresql-ha \
		--set global.postgresql.password=${DEMOSERVER_APPLICATIONMANAGER_POSTGRES_RW_PASSWORD} \
		--set global.postgresql.repmgrPassword=${DEMOSERVER_APPLICATIONMANAGER_POSTGRES_RW_PASSWORD} \
		--set global.pgpool.adminPassword=${DEMOSERVER_APPLICATIONMANAGER_POSTGRES_RW_PASSWORD} \
	...
	helm delete demoserver-applicationmanager -n demoserver --wait
	kubectl delete secret demoserver-applicationmanager -n demoserver
