...
export DEMOSERVER_APPLICATIONMANAGER_VAULT_ROLE_ID=$(vault read -tls-skip-verify auth/approle/role/demoserver/role-id | grep 'role_id ' | awk '{print $2}') 
echo role-id=$DEMOSERVER_APPLICATIONMANAGER_VAULT_ROLE_ID

export DEMOSERVER_APPLICATIONMANAGER_VAULT_SECRET_ID=$(vault write -tls-skip-verify -force auth/approle/role/demoserver/secret-id | grep 'secret_id ' | awk '{print $2}')
echo secret-id=$DEMOSERVER_APPLICATIONMANAGER_VAULT_SECRET_ID