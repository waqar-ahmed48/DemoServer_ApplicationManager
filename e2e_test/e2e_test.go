...
func GetIPAndPort() (string, string) {
	ip := os.Getenv("DEMOSERVER_APPLICATIONMANAGER_SERVICE_IP")
	port := os.Getenv("DEMOSERVER_APPLICATIONMANAGER_SERVICE_PORT")

	if ip == "" {
		ip = "127.0.0.1"

		fmt.Printf("Environment varaiable DEMOSERVER_APPLICATIONMANAGER_SERVICE_IP not set. Setting it to default: %s\n", ip)
	}
